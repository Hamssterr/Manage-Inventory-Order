...
export const OrderPdf = ({ order }: OrderPdfProps) => (
  <Document title={`HoaDon_${order.orderCode}`}>
    <Page size="A4" style={styles.page}>
      ...
      <View style={styles.infoSection}>
        <View style={styles.infoColumn}>
          ...
          <Text style={styles.infoSubValue}>SĐT: {order.customerPhoneSnapshot}</Text>
      ...
      <View style={styles.table}>
        <View style={[styles.tableRow, styles.tableHeader]}>
          ...
          <View style={styles.col2}><Text style={styles.headerText}>Sản phẩm</Text></View>
          <View style={styles.col3}><Text style={styles.headerText}>ĐVT</Text></View>
          <View style={styles.col4}><Text style={styles.headerText}>SL</Text></View>
          <View style={styles.col5}><Text style={styles.headerText}>Đơn giá</Text></View>
          <View style={styles.col6}><Text style={styles.headerText}>Thành tiền</Text></View>
        </View>
        {order.items.map((item, index) => (
          <View key={index} style={styles.tableRow}>
            <View style={styles.col1}><Text style={styles.tableCell}>{index + 1}</Text></View>
            <View style={styles.col2}>
              <Text style={[styles.tableCell, styles.cellBold]}>
                {item.productNameSnapshot}
              </Text>
            </View>
            <View style={styles.col3}><Text style={styles.tableCell}>{item.unitNameSnapshot}</Text></View>
            <View style={styles.col4}><Text style={styles.tableCell}>{item.quantity}</Text></View>
            <View style={styles.col5}>
              <Text style={styles.tableCell}>
                {new Intl.NumberFormat("vi-VN").format(item.priceUnit || 0)}
              </Text>
            </View>
      ...
      <View style={styles.summarySection}>
        ...
        <View style={styles.totalBox}>
          ...
          <Text style={styles.amountInWords}>
            (Bằng chữ: {numberToVietnameseText(order.totalAmount || 0)})
          </Text>
        </View>
      </View>
