...
export const InvoiceForm = ({ order }: InvoiceFormProps) => {
      ...
      <div className="grid grid-cols-2 gap-8 py-8 border-b border-slate-100">
        ...
        <div className="space-y-1 text-right">
          ...
          <p className="text-xs mt-2 italic text-slate-500">
            NV bán hàng:{" "}
            {typeof order.saleId === "object"
              ? order.saleId.displayName
              : "Hệ thống"}
          </p>
        </div>
      </div>
            ...
            {order.items.map((item, index) => (
              ...
              >
                ...
                <td className="py-4 text-sm text-right text-slate-600">
                  {new Intl.NumberFormat("vi-VN").format(item.priceUnit)}
                </td>
      ...
      <div className="mt-4 flex justify-between gap-10">
        ...
        <div className="w-[300px] space-y-3">
          ...
          <p className="text-[10px] text-right text-slate-400 italic">
            (Bằng chữ: {numberToVietnameseText(order.totalAmount)})
          </p>
        </div>
      </div>
